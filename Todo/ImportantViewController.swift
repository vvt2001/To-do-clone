...
class ImportantViewController: UIViewController, UITextFieldDelegate, UIViewControllerTransitioningDelegate {
...
//        let trigger = UNTimeIntervalNotificationTrigger(timeInterval: 5, repeats: false)
    ...
    @IBAction func addTask(_ sender: UIButton){
        if let taskName = addTaskField.text, !taskName.isEmpty{
            ...
            if let index = taskStore.importantTask.lastIndex(where: {$0.getId() == newTask.getId()}){
        ...
        if isSelectedAll == false{
            ...
            if taskStore.importantTask.count > 0{
                for i in 0...(taskStore.importantTask.count - 1){
                    taskStore.importantTask[i].setIsSelected(isSelected: true)
                }
            }
...
extension ImportantViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.createCell(name: taskStore.importantTask[indexPath.row].getName())
        return cell
    }
